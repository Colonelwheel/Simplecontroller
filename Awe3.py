...
# Set up logging with rotation to avoid large log files
logging.basicConfig(
    level=logging.INFO,
...
PORT = 9001       # Port used in your Android app
USE_UDP = True    # Enable UDP support alongside TCP
...
last_position_time = {}  # Track last time a position was sent for each control
position_throttle_ms = 8  # Minimum time between position updates (ms)
command_queue = deque(maxlen=100)  # Queue for commands to avoid processing bottlenecks
queue_lock = threading.Lock()  # Lock for thread-safe queue access

# Optimize player prefixes
PLAYER1_PREFIX = "player1:"
PLAYER2_PREFIX = "player2:"
...
def handle_stick_input(x, y, stick_type="LEFT"):
    ...
    # Apply deadzone if very close to center (reduces jitter)
    if abs(x) < 0.05 and abs(y) < 0.05:
        x, y = 0, 0
    
    try:
        if stick_type == "LEFT":
            gamepad.left_joystick_float(x_value_float=x, y_value_float=-y)  # Y is inverted
        else:
            gamepad.right_joystick_float(x_value_float=x, y_value_float=-y)  # Y is inverted
        
        gamepad.update()
        
        # Reduce logging frequency for position updates to improve performance
        if logger.isEnabledFor(logging.DEBUG):  # Only log at debug level
            logger.debug(f"Stick {stick_type}: x={x:.2f}, y={y:.2f}")
    except Exception as e:
    ...
    try:
        ...
        if trigger == "LEFT":
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"Left trigger: {value:.2f}")
        else:
            gamepad.right_trigger_float(value_float=value)
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"Right trigger: {value:.2f}")
...
def handle_button_press(command):
    ...
    # Xbox controller buttons - with shortened syntax for better performance
    xbox_buttons = {
        # Original syntax
        ...
        # Shorter Xbox syntax (optimized for less string processing)
    ...
    if command in xbox_buttons:
        try:
            ...
            if not command.endswith("_HOLD"):
                ...
                release_timer.start()
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"Scheduled auto-release for {command}")
            else:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"Hold mode - no auto-release for {command}")
        except Exception as e:
...
def process_command(data, control_id=None):
    ...
    # Only log at lower frequency for position updates to reduce log volume
    if not (data.startswith("STICK") or data.startswith("LS:") or data.startswith("RS:") or
            data.startswith("TOUCHPAD") or data.startswith("POS:")):
    ...
    player_id = None
    if data.startswith(PLAYER1_PREFIX):
        player_id = 1
        data = data[len(PLAYER1_PREFIX):]
    elif data.startswith(PLAYER2_PREFIX):
        player_id = 2
        data = data[len(PLAYER2_PREFIX):]
    
    # Store control ID for throttling if provided
    if control_id is None:
        control_id = f"player{player_id}_{data.split(':')[0] if ':' in data else 'cmd'}"
    
    # Skip position updates that arrive too frequently for the same control
    if (data.startswith("STICK") or data.startswith("LS:") or data.startswith("RS:") or
            data.startswith("TOUCHPAD") or data.startswith("POS:")):
...
def command_processor():
    """Process commands from the queue to prevent blocking network IO"""
    while True:
        try:
            # Get next command if available, otherwise wait a bit
            try:
                with queue_lock:
                    if command_queue:
                        command, control_id = command_queue.popleft()
                    else:
                        command, control_id = None, None
            except Exception:
                command, control_id = None, None
            
            if command:
                process_command(command, control_id)
            else:
                # Sleep a tiny bit to avoid spinning the CPU when idle
                time.sleep(0.001)
        except Exception as e:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                logger.warning(f"Received invalid UTF-8 data from {addr}")
                continue
            ...
            for line in lines[:-1]:
                if line.strip():
                    # Queue command for processing instead of blocking network IO
                    with queue_lock:
                        control_id = f"tcp_{addr[0]}_{addr[1]}_{line.split(':')[0] if ':' in line else 'cmd'}"
                        command_queue.append((line, control_id))
...
def udp_server():
    """Run UDP server to receive datagrams"""
    if not USE_UDP:
        return
    ...
    try:
        ...
        while True:
            try:
                ...
                ready, _, _ = select.select([udp_socket], [], [], 0.01)
                if not ready:
                ...
                try:
                    ...
                    control_id = f"udp_{addr[0]}_{addr[1]}_{message.split(':')[0] if ':' in message else 'cmd'}"
                    with queue_lock:
                        command_queue.append((message, control_id))
                except UnicodeDecodeError:
                    logger.warning(f"Received invalid UTF-8 data via UDP from {addr}")
            except BlockingIOError:
                # No data available, just continue
                pass
            except Exception as e:
                logger.error(f"Error in UDP server: {str(e)}")
        ...
        try:
            ...
            pass
        logger.warning("UDP server stopped")
    ...
    try:
        ...
        while True:
            try:
                ...
                client_thread = threading.Thread(target=tcp_client_handler, args=(conn, addr))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Start the UDP server in a separate thread if enabled
        if USE_UDP:

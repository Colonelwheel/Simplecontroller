...
class ControlViewHelper(
    ...
    /**
     * Parse compact analog-stick button syntax.
     *
     * Examples:
     *   LS:R50   -> left stick 50% right
     *   LS:UR100 -> left stick 100% up-right
     *   RS:BL25  -> right stick 25% bottom-left
     *
     * Direction codes: U, D, L, R, UR, UL, BL, BR.
     * Percentage is 0..100. If omitted, 100 is used.
     */
    private data class CompactStickCommand(
        val stickName: String,
        val x: Float,
        val y: Float
    )

    private fun parseCompactStickCommand(command: String): CompactStickCommand? {
        val match = Regex(
            pattern = """^(LS|RS):(UR|UL|BL|BR|U|D|L|R)(\d{1,3}(?:\.\d+)?)?$""",
            option = RegexOption.IGNORE_CASE
        ).matchEntire(command.trim()) ?: return null

        val stick = match.groupValues[1].uppercase()
        val direction = match.groupValues[2].uppercase()
        val percentText = match.groupValues[3]
        val amount = ((percentText.toFloatOrNull() ?: 100f).coerceIn(0f, 100f)) / 100f

        // Android stick coordinates use negative Y for up and positive Y for down.
        ...
        return CompactStickCommand(
            stickName = if (stick == "RS") "STICK_R" else "STICK_L",
            x = x,
            y = y
        )
    }
    ...
    private fun sendCommand(command: String) {
        ...
        // ───── Compact stick button syntax: LS:R50 / RS:UL100 ─────
        parseCompactStickCommand(command)?.let { stick ->
            Log.d(
                "ControlViewHelper",
                "Compact stick command: ${stick.stickName} x=${stick.x}, y=${stick.y}"
            )
            UdpClient.sendStickPosition(stick.stickName, stick.x, stick.y)
            return
        }
    ...
    fun releaseLatched() {
        ...
        model.payload.split(',', ' ')
            ...
            .forEach { cmd ->
                ...
                } else if (parseCompactStickCommand(cmd) != null) {
                    // A button-assigned stick direction behaves like a held analog stick:
                    // releasing/unlatching the button returns that stick to center.
                    val stick = parseCompactStickCommand(cmd)!!
                    Log.d(
                        "ControlViewHelper",
                        "Centering compact stick command on release: ${stick.stickName}"
                    )
                    UdpClient.sendStickPosition(stick.stickName, 0f, 0f)
